...
export default function PlaceholderArt({
  ...
  return (
    <div
      className={`flex flex-col items-center justify-center gap-2 bg-gradient-to-br from-sage-100 via-cream-200 to-gold-100 text-link ${className}`}
    >
      <Feather className="h-8 w-8 opacity-60" strokeWidth={1.5} />
      {label ? (
        <span className="px-4 text-center text-xs tracking-wide text-link/70">
          {label}
        </span>
      ) : null}
    </div>
